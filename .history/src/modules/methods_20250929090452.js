...
export default class Methods {
  ...
  async loadModalInfo(index, commentsAPI) {
    ...
    // add trailer button/link if available
    const existingTrailer = document.getElementById('watch-trailer');
    if (existingTrailer && existingTrailer.parentNode) existingTrailer.parentNode.removeChild(existingTrailer);
    if (anime.trailer) {
      const trailerBtn = document.createElement('a');
      trailerBtn.id = 'watch-trailer';
      trailerBtn.className = 'cta';
      ...
      if (right) right.insertBefore(trailerBtn, right.querySelector('div'));
    }
