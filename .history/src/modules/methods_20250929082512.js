...
export default class Methods {
  ...
  async loadData(movieList, animeAPI, likesAPI) {
    ...
    list.forEach((anime, i) => {
      ...
      const cleanSynopsis = Methods.sanitizeSynopsis(anime.synopsis || '');
      const item = {
        id: i,
        title: anime.title,
  ...
  // render with read-more if too long
  renderWithReadMore(description, raw, 400);
  ...
  if (badge) badge.innerText = anime.title.length > 28 ? `${anime.title.slice(0, 25)}…` : anime.title;
    ...
    function renderWithReadMore(container, text, maxChars = 300) {
      // clear
      container.innerHTML = '';
