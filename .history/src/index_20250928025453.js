...
// like animation: when a like button is clicked, animate badge and spawn bursts
container.addEventListener('click', (ev) => {
  const likeBtn = ev.target.closest('.like-btn');
  if (!likeBtn) return;
  // find likes span within card
  const card = likeBtn.closest('.movie-card');
  const span = card && card.querySelector('[id^="span-"]');
  if (!span) return;
  const badge = document.createElement('div');
  badge.className = 'like-badge';
  badge.innerHTML = `<span class="heart-pop">❤</span> <span class="count">${span.innerText}</span>`;
  // position badge near button
  ...
  // spawn a few tiny bursts for visual delight
  for (let i = 0; i < 4; i += 1) {
    const burst = document.createElement('div');
    burst.className = 'heart-burst';
    burst.style.left = `${8 + Math.random() * 24}px`;
    burst.style.top = `${8 + Math.random() * 24}px`;
    likeBtn.appendChild(burst);
    // play animation
    requestAnimationFrame(() => burst.classList.add('burst-anim'));
