...
(async function initApp() {
  ...
  // paging state
  let currentPage = 1;
  let currentQuery = null; // for search pagination
...
const serverSearch = debounce(async (e) => {
  ...
  const url = `https://api.jikan.moe/v4/anime?q=${encodeURIComponent(q)}&limit=24`;
  await methods.loadData(container, url, likesAPI);
}, 450);
