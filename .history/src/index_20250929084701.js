...
// Prefer newest first: if API supports sortBy=releaseDate and sortOrder=desc, use that; otherwise fallback to ranking
const animesAPI = 'https://anime-db.p.rapidapi.com/anime?page=1&size=20&genres=Award%20Winning%2CAction%2CHorror%2CSports%2CSupernatural%2CFantasy%2CDrama%2CComedy%2CAdventure%2CRomance%2CSci-Fi&sortBy=releaseDate&sortOrder=desc';
...
(async function initApp() {
  ...
  try {
    ...
    if (methods.animes && methods.animes.length > 0) {
      const latest = methods.animes[0];
      if (featured && latest.image) featured.setAttribute('src', latest.image);
      if (featuredTitle) featuredTitle.innerText = latest.title || 'Featured Anime';
      // wire play button to open modal for the featured item (index 0)
      if (playBtn) playBtn.addEventListener('click', () => {
        methods.loadModalInfo(0, commentsAPI);
        lastFocused = document.activeElement;
        document.documentElement.classList.add('modal-open');
        setTimeout(() => closeModal.focus(), 0);
      });
      // wire favorite button to toggle local favs and visual state
      if (favBtn) favBtn.addEventListener('click', () => {
        const fakeCard = { querySelector: (sel) => (sel === '.fav-btn' ? favBtn : null) };
        // reuse toggleFav but we need the card element reference; create a small shim
        const cardEl = document.createElement('div');
        cardEl.dataset.id = 0;
        cardEl.className = 'movie-card';
        // persist favorites in localStorage manually for featured
        const favs = JSON.parse(localStorage.getItem('favs') || '[]');
        const existing = favs.indexOf(0);
        if (existing === -1) {
          favs.push(0);
